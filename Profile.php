<?php
	...
	if (isset($_SESSION['username'])){
		...
		$final = $load_user->getUser($userid);
		/* $db =new DB();
		$sql="SELECT * FROM logins where reg_no=?";
		$state = mysqli_prepare($db->getConnection(), $sql);
		$state->bind_param("s", $_SESSION['username']);
		$state->execute() ;
...
<body>
	<form method="post">
		...
		<fieldset><div class="container"style="background-color:	#39E5E0">
			...
			<label  class="text-white" >Name </label></div><div class="col-md-4"><label  class="text-white" >: 	<?php echo $final["fname"] ?> <?php echo $final["lname"] ?></label></div> 
			<div class="col-md-3"><img src="Images/<?php echo $final["reg_no"]; ?>" class="img-fluid" alt="Upload Your Photo"><br> </div></div>
			<div class="row"><div class="col-md-2">
